...
namespace AxoCover.Runner
{
    ...
    public class TestDiscoveryService : ITestDiscoveryService
    {
        ...
        private Type[] GetTypesDiscover(string adapterSource)
        {
            var assembly = Assembly.Load(AssemblyName.GetAssemblyName(adapterSource));
            var typeDiscover = typeof(ITestDiscoverer);
            ...
            return types
                .Where(T => T != null && T.GetInterface(typeDiscover.Name) != null)
                .ToArray();// ;.FindImplementers<ITestDiscoverer>();
        }
        ...
        private ITestDiscoverer[] LoadDiscoverers(string adapterSource)
        {
            ...
            try
            {
                ...
                var implementers = GetTypesDiscover(adapterSource);

                foreach (var implementer in implementers)
                {
                    try
                    {
                        var testDiscoverer = Activator.CreateInstance(implementer);
                        testDiscoverers.Add(testDiscoverer as ITestDiscoverer);
